...
export function ChangeProductsPrice({
  ...
  const { register, watch, getValues } = useForm<newChangeProductPrice>({
  ...
  if (!isChangingProductPrice) {
    return null;
  }

  return (
    <Dialog.Root
      open={isChangingProductPrice}
      onOpenChange={open => setIsChangingProductPrice(open)}
    >
      <Dialog.Trigger />
      <Dialog.Portal>
        <Dialog.Overlay
          onClick={() => setIsChangingProductPrice(false)}
          className="bg-black data-[state=open]:animate-overlayShow opacity-60 fixed inset-0"
        />

        <Dialog.Content className="data-[state=open]:animate-contentShow bg-white fixed top-[40%] left-[50%] w-[90vw] max-w-[400px] translate-x-[-50%] translate-y-[-50%] rounded-[6px] p-[25px] shadow-[hsl(206_22%_7%_/_35%)_0px_10px_38px_-10px,_hsl(206_22%_7%_/_20%)_0px_10px_20px_-15px] focus:outline-none">
