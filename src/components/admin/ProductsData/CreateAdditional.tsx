...
const newAdditionalValidationSchema = zod
  .object({
...
export function CreateAdditional({}: iCreateAdditionalProps) {
  const { restaurant, additional_categories, setAdditionals } =
  ...
  const handleCreateAdditional = async (data: newAdditional) => {
    ...
    const { data: additonal, status } = await api.post(
      `api/additionals/${restaurant.id}`,
      {
        name,
        price,
    ...
    <div className={``}>
      <Dialog.Root>
        ...
        <Dialog.Portal>
          ...
          >
            <form onSubmit={handleSubmit(handleCreateAdditional)} className="">
