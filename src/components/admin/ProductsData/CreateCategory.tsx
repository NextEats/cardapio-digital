...
import { iProductCategories } from "@/src/types/types";
...
export function CreateCategory({ categoryType }: iCreateCategoryProps) {
    ...
    const { register, getValues, watch, setError, setValue, handleSubmit, formState: { errors, isSubmitting } } = useForm<newCategory>({
                    ...
                    <form onSubmit={handleSubmit(handleCreateCategory)}>
                        ...
                        <div className="flex flex-wrap items-center gap-3 mb-4 mt-6">
                            {sequence.map(sequence => {
                                const busyoOrder = categories.some(c => c.category_order === sequence)
                                return <button
                                    key={sequence}
                                    disabled={busyoOrder}
                                    type="button"
                                    ...
                                    ${busyoOrder ? 'bg-gray-300' :
                                            sequence === newOrder ? 'bg-yellow-400' : 'bg-blue-400'}
                                    `}
                                >
                                    {sequence}
                                </button>
