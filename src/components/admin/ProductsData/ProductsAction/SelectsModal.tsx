...
export function SelectsModal({ type }: iSelectsProps) {
  const {
    selectsState,
  ...
  const [selectToCreateOption, setSelectToCreateOption] = useState<
    iSelect['data'] | null
  >(null);
  const [isUpadatingSelect, setIsUpadatingSelect] = useState<
    iSelect['data'] | null
  >(null);
  const [updateOption, setUpdateOption] = useState<
    iProductOption['data'] | null
  >(null);
  ...
  const [product_selects, setProduct_selects] = useState<
  ...
  const handleDeleteSelect = async (select: iSelect['data']) => {
    ...
    if (data && data.length > 0) {
      const isConfirmed = confirm(
        'Esta personalização está a alguns produtos. Tem certeza de que deseja excluí-la?'
      );
      if (isConfirmed) {
        await supabase
          .from('product_selects')
  ...
  return (
    <div className={``}>
      <UpdateSelect
        isUpadatingSelect={isUpadatingSelect}
        setIsUpadatingSelect={setIsUpadatingSelect}
      />
      <UpdateProductOption
        ...
        <Dialog.Trigger asChild>
          {type === 'select_selects' ? (
            ...
            <button className="px-[15px] font-medium leading-none outline-none text-brand-dark-orange truncate">
              {selects.length}{' '}
              {selects.length === 1 ? 'personalização' : 'personalizações'}
            </button>
          )}
        </Dialog.Trigger>
        <Dialog.Portal>
                        ...
                        <div className="flex items-center gap-4">
                          {select.has_default_price ? (
                            <span className="">
                              {' '}
                              R${' '}
                              {select.price?.toLocaleString('pt-BR', {
                          ...
                          <DropdownMenu.Root>
                            ...
                            <DropdownMenu.Portal>
                              ...
                              >
                                ...
                                <DropdownMenu.Item
                                  onClick={() =>
                                    handleChangeSelectStatus(select)
                                  }
                                  className="group text-[13px] leading-none text-violet11 rounded-[3px] flex items-center gap-3 hover:bg-white-blue cursor-pointer h-9 px-[5px] relative pl-[25px]"
                                >
                                  <BsFillPencilFill size={16} />
                          ...
                          return (
                            <div key={product_option.id}>
                              {/* <label
                                                    ...
                                                    > */}
                              <div
                                className={`w-[100px] h-[100px] rounded-lg relative cursor-pointer`}
                              >
                                <DropdownMenu.Root>
                                  ...
                                  <DropdownMenu.Portal>
                                    ...
                                    >
                                      <DropdownMenu.Item
                                        onClick={() =>
                                          setUpdateOption(product_option)
                                        }
                                        className="group text-[13px] leading-none text-violet11 rounded-[3px] flex items-center gap-3 hover:bg-white-blue cursor-pointer h-9 px-[5px]  relative pl-[25px]"
                                      >
                                        <BsFillPencilFill size={16} />
                                      ...
                                      <DropdownMenu.Item
                                        onClick={() =>
                                          handleDeleteOption(product_option)
                                        }
                                        className="group text-[13px] leading-none text-violet11 rounded-[3px] flex items-center gap-3 hover:bg-white-blue cursor-pointer h-9 px-[5px] relative pl-[25px]"
                                      >
                                        <BsFillPencilFill size={16} />
                                ...
                                <div
                                  className={`w-full h-full absolute rounded-lg z-10 bg-gradient-to-t from-[#000000ff] via-[#00000063] to-[#00000000]`}
                                ></div>
                                {!product_option.active ? (
