...
const updateAdditionalValidationSchema = zod
  .object({
...
export function UpdateAdditional({}: iUpdateAdditionalProps) {
  ...
  const handleUpdateAdditional = async (data: updateAdditionalType) => {
    ...
    if (typeof picture[0] === 'object') {
      const { path } = getPathByPictureUrl(updateAdditional?.picture_url!);
    ...
    const { data: additonal, status } = await supabase
      .from('additionals')
      .update({
        name,
        picture_url,
  ...
  return (
    <Dialog.Root open={updateAdditional !== null}>
      ...
      <Dialog.Portal>
        ...
        />
        <Dialog.Content
          className="data-[state=open]:animate-contentShow fixed top-0 right-0 3xs:top-[30%] 3xs:left-[50%] h-screen  w-screen
        ...
        >
          <form onSubmit={handleSubmit(handleUpdateAdditional)} className="">
            <Dialog.Title className="text-mauve12 flex flex-1 items-center justify-between m-0 text-[17px] font-medium mb-4">
              Criar Adicional
              <button
                disabled={isSubmitting}
                className="px-6 py-1 text-white rounded-full bg-blue-400 disabled:bg-gray-400 "
                type="submit"
              >
                salvar
              </button>
