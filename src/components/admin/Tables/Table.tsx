...
export default function Table({ table }: iTableProps) {
    ...
    const thisTable = ordersTables.filter((elem, index) => elem.tables.id === table.id)
    // console.log(ordersTables)
    // console.log("thisTable", thisTable)
    // console.log("table", ordersTables.filter((elem, index) => elem.table_id === table.id))
    // console.log("elem", ordersTables.map((elem, index) => console.log(elem)))

    return (
        <div className={`flex flex-1 flex-col border-l-8 rounded-md bg-white shadow-md py-2 pr-3 pl-4 `}>
