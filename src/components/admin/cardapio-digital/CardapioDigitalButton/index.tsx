...
interface iButtonProps {
    onClick?: () => void,
    Icon?: ReactNode,
    name?: "Criar nova categoria" | "Novo" | "Cancelar" | "Editar" | "Adicionar" | "Adicionar novo item",
    h?: string,
    w?: string,
    disabled?: boolean,
}
...
export function CardapioDigitalButton({ name = 'Adicionar', onClick, h, w, Icon, disabled }: iButtonProps) {
    return (
        ...
        type="submit"
        onClick={onClick} 
        className={` flex items-center gap-1 justify-center text-white leading-5 font-semibold rounded transition-all ease-in-out ${w} ${h}
