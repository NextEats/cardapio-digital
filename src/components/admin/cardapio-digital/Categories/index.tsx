...
export default function Categories({ productCategories, setEditCategory, setViewCategory, products }: iCategoriesProps) {
    return (
            ...
            <table className="w-full border-collapse">
                <tbody className="">
                    {productCategories.map(category => {
                        ...
                        const filterProductByCategory = products.filter(product => product.category_id === category.id)
                        ...
                        return (
                            <tr key={category.id} className="">
                                ...
                                <td className=" border-t-4 border-solid border-transparent text-base text-gray-700 font-medium"> { filterProductByCategory.length } itens </td>
                                <td className=" border-t-4 border-solid border-transparent text-right">
                                    <div className="flex items-center justify-end">
                                        <button 
                                        onClick={() => setViewCategory({ isViewing: true, categoryId: category.id, categoryName: category.name })}
