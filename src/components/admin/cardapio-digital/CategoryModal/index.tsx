...
import { iInsertProductCategory, iProductCategories, iProducts, iRestaurantWithFKData } from "@/src/types/types";
...
export function CategoryModal({
  ...
  const { register, reset, getValues, setValue, watch } = useForm<NewCategoryFormData>(
    {
  ...
  async function handleNewCategory() {
    ...
    if (categoryOrdervalue <= 0) {
      alert("Insira somente valores acima de 0 para a ordem da categoria.")
      return
    }
    if (productCategories.some(c => c.category_order === categoryOrdervalue)) {
      alert("Já existe uma categoria nessa posição, por favor escolha outro número.")
      return
    }
  ...
  useEffect(() => {
    ...
    if (editCategory.isEditing)
      console.log(editCategory.categoryData.category_order, editCategory.categoryData.name)
  ...
  const numbersOfCategoryOrder = productCategories.map(category => category.category_order).sort((a, b) => {
    if (a > b) return 1
    if (a < b) return -1
    return 0
  })
        ...
        <>
          ...
          >
            <form className="w-full">
              ...
              <input
                type="number"
                {...register("categoryOrder", { valueAsNumber: true })}
              ...
              />
              {productCategories.some(c => c.category_order === categoryOrdervalue) &&
                editCategory.categoryData.category_order !== categoryOrdervalue
                ?
                <span className="text-red-500 text-sm font-normal leading-1">Já existe uma categoria nessa posição, por favor escolha outro número.</span>
                : null}

              <div className="flex flex-wrap items-center gap-3 mb-4 mt-6">{
                ascendingSequence.map(sequence => {
                  if (productCategories.some(pc => pc.category_order === sequence)) {
                    return (<button key={sequence}
                      className={`text-base font-semibold text-gray-700 w-6 h-6 rounded cursor-not-allowed  ${editCategory.categoryData.category_order === sequence ?
                        'bg-green-400'
                        : 'bg-gray-400'}`}> {sequence}</button>)
                  }
                  return (<button
                    key={sequence}
                    onClick={(e) => {
              ...
              } <span className="text-2xl font-semibold text-gray-700 leading-4">...</span>
              </div>
