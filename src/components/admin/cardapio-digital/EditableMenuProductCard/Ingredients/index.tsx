...
import {
  ...
  removeOptionFromIngredientAction,
...
import {
  createIngredientIfIsUpdatingProduct,
  createProductOptionIfIsUpdatingProduct,
  deleteIngredientIfIsUpdatingProduct,
  deleteProductOptionIfIsUpdatingProduct,
  supabase,
} from "@/src/server/api";
import { iInsertSelects } from "@/src/types/types";
...
export function Igredient({
  state,
  dispatch,
  selects,
  productId,
}: iIgradientsCardProps) {
  const [isAddingNewIngradientState, setIsAddingNewIngradientState] =
    useState(false);
  const [isUpdatingIngradientNameState, setIsUpdatingIngradientNameState] =
    useState("");
  const [showModalOption, setShowModalOption] = useState("");
  const [error, setError] = useState<"Esse Ingrediente já existe" | "">("");

  const { register, handleSubmit, reset, setValue, getValues } =
    useForm<NewIngredientFormData>({
  ...
  async function handleAddNewIngredient(formData: NewIngredientFormData) {
    if (
      selects.some(
        (select) =>
          select.name.toLowerCase() === formData.ingredientName.toLowerCase()
      ) ||
    ...
    if (
      state.ingredients.some(
        (ingredient) =>
          ...
          data.updateIngredientName.toLowerCase()
      )
    ) {
      setError("Esse Ingrediente já existe");
      return;
  ...
  function handleAddNewOptionToIngredient(ingredientId: number) {
    ...
    dispatch(
      setAddNewOption(optionName, optionPicture_url, ingredientId.toString())
    );
    setShowModalOption("");
  }

  function removeOptionFromIngredient(
    ingredientId: number,
    optionName: string,
    optionId: number
  ) {
    ...
    <div>
            ...
            <div className="mb-4">
              {isUpdatingIngradientNameState === ingredient?.name ? (
                <div className="flex items-center gap-3">
                  <form
                    onSubmit={handleSubmit(handleUpdateIngredientName)}
                    className="w-full"
                  >
                    <div className="w-full flex items-center gap-2">
                      ...
                      <input
                        ...
                        hidden
                        value={ingredient.id}
                        {...register("ingredientId", { value: ingredient.id })}
                      />
                      <button
                        type="button"
                    ...
                    <span className="text-xs font-normal text-red-500">
                      {" "}
                      {error === "Esse Ingrediente já existe"
                        ? error
                        : null}{" "}
                    </span>
                  </form>
              ...
              {showModalOption === ingredient?.name && (
                <div className="w-56 h-60 p-4 absolute z-50 top-0 right-1/2 translate-x-1/2 rounded-md bg-white shadow-md">
                  ...
                  <div className="w-full flex items-center gap-2 mt-6">
                    ...
                    <button
                      onClick={() =>
                        handleAddNewOptionToIngredient(ingredient?.id!)
                      }
      ...
      {isAddingNewIngradientState ? (
        ...
        >
          ...
          <span className="text-xs font-normal text-red-500">
            {" "}
            {error === "Esse Ingrediente já existe" ? error : null}{" "}
          </span>
        </form>
      ) : null}
      ...
      {state.isViewingUpdatingOrAdding !== "VIEWING" ? (
        <div className="w-full flex items-center justify-end mt-6">
          <CardapioDigitalButton
            onClick={() =>
              setIsAddingNewIngradient(
                isAddingNewIngradientState ? false : true
              )
            }
            name={!isAddingNewIngradientState ? "Adicionar" : "Cancelar"}
            h="h-8"
            w="w-28"
          />
        </div>
