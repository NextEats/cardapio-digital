...
import { iInsertOrderProduct } from '@/src/types/iOrders';
...
export function DropdownMenuObservation({
  orderProduct,
  additionals,
}: iDropdownMenuObservationProps) {
  const orders = orderProduct.selects_data as
    ...
    | undefined
    | null;
  const additionalsData = orderProduct.additionals_data as {
    quantity: number;
  ...
  const additionalsDataFiltered =
    additionalsData !== null
      ? additionalsData.reduce(
          (
            acc: { additional: iAdditional['data']; quantity: number }[],
            item
          ) => {
    ...
    <DropdownMenu.Root>
      ...
      <DropdownMenu.Portal>
        <DropdownMenu.Content
          data-state="top"
          className="rounded max-w-[350px] sm:max-w-[400px] bg-white p-3 shadow-md "
        >
            ...
            {additionalsDataFiltered.map(additional => {
              return (
                <div
                  ...
                  className="flex items-center justify-between pr-2 text-sm font-normal"
                >
                  <span>
                    {' '}
