import { iAdditional, iAdditionals, iInsertOrdersProducts, iSelects } from "@/src/types/types";
            ...
            {result.map((product) => {
                ...
                return (
                    <div key={product.id} className="w-full">
                        <div className={`w-full flex items-center justify-between ${textStyles} mt-1`}>
                            <span>
                                {product.count} - <strong className=''>{product.name}</strong>
                            </span>
                        ...
                        <div className='flex'>
                            {
                                <>
                                    {selectsDada!.map(s => {
                                        return <div key={s.id} className='flex flex-row'>
                                            {
                                                s.options.map(o => {
                                                    return <span key={o.id} className={`text- font-normal ml-2 ${textStyles} `}>
                                                        {o.name}{','}
                                                    </span>
                                                })
                                            }
