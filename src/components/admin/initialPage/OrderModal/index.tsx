...
import { iInsertProducts, iProducts } from '@/src/types/iProducts';
...
export function OrderModal({
  ...
  const orderDateFormated = format(new Date(`${orderFound?.created_at}`), 'P', {
    locale: ptBR,
  });
            ...
            <div className="fixed mt-12 right-1/2 translate-x-1/2 p-4 bg-[#fb3d3a] w-[298px] rounded-lg">
              ...
              >
                ...
                <div>
                  ...
                  <p className={`${textStyles}`}>
                    Pagamento:{' '}
                    <strong>{orderFound?.payment_methods.name}</strong>
                  </p>
                  {ordersTablesFound ? (
                    <p className={`${textStyles}`}>
                      Nome da mesa :{' '}
                  ...
                  {orderFound?.payment_methods.id == 5 ? (
                    <p className={`${textStyles}`}>
                      Troco:
                      <strong>R$ {orderFound?.change_value} </strong>
                    </p>
                  ) : null}
                </div>
                  ...
                  <>
                    ...
                    <div>
                      <p className={`${textStyles} text-left `}>
                        ...
                        Telefone:{' '}
                        <strong>{orderFound?.clients.contacts?.phone} </strong>
                      </p>
                      <p className={`${textStyles}`}>
                        Endereço:{' '}
                        <strong>
                          ...
                          {orderFound?.clients.addresses?.complement}-{' '}
                ...
                <ProductsDetails
                  productsFiltered={productsFiltered! as iProducts['data']}
                  additionals={additionals}
                  result={result}
                  orderProductFiltered={orderProductFiltered}
                  selects={selects}
                ...
                <div>
                  {orderFound?.payment_methods.name !== 'MESA' ? (
                ...
                <div className="flex flex-1 flex-col items-center justify-end gap-3 mt-5 hideButtonToPrint">
                  {!ordersGroupedByOrderStatus[
                    'em análise'
                  ] ? null : ordersGroupedByOrderStatus['em análise'].some(
                      o => o.id === orderFound?.id
                    ) ? (
                    <ReactToPrint
                      copyStyles={true}
