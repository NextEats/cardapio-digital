...
import { iProduct, iProductCategories, iProducts } from '@/src/types/iProducts';
...
export function BarChart({ globalValuesData }: iBarCgart) {
  ...
  const top12ProductsThatHaveBeenDelivered = productsThatHaveBeenDeliveredArray
    .sort(
      (menor, maior) =>
        maior.numberOfProductsPurchased - menor.numberOfProductsPurchased
    )
    .slice(0, 10);
              ...
              <div className="h-16 flex flex-col items-center justify-between">
                ...
                <HoverCard.Root openDelay={200} closeDelay={0}>
                  <HoverCard.Trigger asChild>
                    {product.product && product.product.picture_url ? (
                      <Image
                        ...
                        <div className="flex flex-col gap-3">
                          ...
                          <div className="flex gap-4">
                            <span className="text-lg font-bold text-gray-700">
                              {' '}
                              {product.numberOfProductsPurchased} de{' '}
                              {ordersProductWithStatusEntrega.length}{' '}
                            </span>{' '}
                            ...
                            <span className="text-lg font-bold text-gray-700">
                              {' '}
                              {percentageOfProducts.toFixed(2)}%{' '}
                            </span>{' '}
                            <span className="text-lg font-medium text-gray-500">
                              Percentual do produto vendido
                            </span>
                          </div>
