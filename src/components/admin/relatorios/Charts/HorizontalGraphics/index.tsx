...
export function HorizontalGraphics({ globalValuesData }: iHorizontalGraphicsProps) {
    ...
    for (let i = 0; i < productsPurchased.length; i++) {
        ...
        for (let j = 0; j < productCategories.length; j++) {
            ...
            if (productCategories[j].id === category_id) {
                ...
                if (numberOfProductsSoldInEachCategory[productCategories[j].name] === undefined) {
                    numberOfProductsSoldInEachCategory[productCategories[j].name] = { name: productCategories[j].name, count: 0 };
                }
