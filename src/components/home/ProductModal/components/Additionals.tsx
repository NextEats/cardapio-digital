...
import { iAdditional, iAdditionalCategories, iAdditionals } from '@/src/types/types';
...
export default function Additionals({
    ...
    // const [additionalsCategories, setAdditionalsCategories] = useState<iAdditionalCategories["data"] | null>(null)
    const [additionalsGrupedCategory, setAdditionalsGrupedCategory] = useState<{
        categoryName: string,
    ...
    useEffect(() => {
        async function getAdditionalsCategories() {
            const getAddCat = await supabase.from("additional_categories").select().eq('restaurant_id', restaurant?.id)
            // setAdditionalsCategories(getAddCat.data)
            if (getAddCat.data === null) return
            for (let i = 0; i < getAddCat.data?.length!; i++) {
                const additionalsFiltered = state.additionals.filter(add => add.additional_category_id === getAddCat.data[i].id)
                if (additionalsGrupedCategory?.some(addC => addC.categoryName === getAddCat.data[i].name!)) return
                setAdditionalsGrupedCategory(state => [...state!, {
                    categoryName: getAddCat.data[i].name!,
                    categoryOrder: getAddCat.data[i].category_order!,
                    additionals: additionalsFiltered,
                }])
            }
            setAdditionalsGrupedCategory(state => {
                const sortedCategories = [...state!].sort((a, b) => a.categoryOrder - b.categoryOrder);
    ...
    return (
        <div>
            ...
            {additionalsGrupedCategory ? additionalsGrupedCategory.map((additionalByCategory, index) => {
                if (additionalByCategory.additionals.length === 0) return
                return (
                    <div key={index}>
                        <p className='text-lg mt-4'>{additionalByCategory.categoryName}</p>
                        {
                            additionalByCategory.additionals.map(additional => {
