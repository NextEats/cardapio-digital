...
export default function Product({ showProduct, setShowProduct }: IProductProps) {
    ...
    const [ ingredientsOption, setIngredientsOption ] = useState<IIngredientOptionsData[]>([])
    ...
    return (
        <>
            {showProduct && <div className={`w-[345px] pb-9 px-4 bg-white absolute top-0 right-0 z-10 overflow-y-auto shadow-2xl`}>
                <BsArrowLeftCircle
                    className="my-8 cursor-pointer" size={30}
                ...
                <div className="mb-9">
                    ...
                    <p className="font-normal text-sm text-gray-800 " >
                        Molho especial da casa, hambúrguer artesanal 150g feito com
                        carne bovina de alta qualidade, cebola roxa, picles, tomate, alface e queijo prato
                    </p>
                </div>
                                            ...
                                            return (
                                                <label onClick={() => getAdditional()} key={category.id}>
                                                    ...
                                                    <div className="w-[99px] h-[94px] rounded-lg relative cursor-pointer "  >
                                                        ...
                                                        <span className="absolute bottom-1 left-1 z-20 text-white-300 text-sm font-medium " >{category.name}</span>
                                                        <Image
                                                            src={category.picture}
                                                            alt="carne malpassada"
                                                            className={`wi-full h-full relative rounded-lg object-cover ${ !category.isSelected && "contrast-50"}`}
                                                            width={326}
                                                            height={358}
                                                        />
                                                    </div>
                                ...
                                <div className="flex flex-1 items-center justify-between pr-4 shadow-md rounded-md bg-white-300" >
                                    <div className="flex items-center gap-3" >
                                        ...
                                        <div className="">
                                            ...
                                            <p className="font-semibold text-xs text-black">R$ {item.price}</p>
                                        </div>
                                    </div>
                                    <BsFillPlusCircleFill className="cursor-pointer" color="3A3A3A" size={25} />
                                </div>
                            </div>
                        )
                    })}
                ...
                <div className="flex flex-1 items-center">
                    ...
                    <div className=" h-10 flex flex-1 items-center gap-2 bg-gray-700 shadow-md rounded-md ">
                        ...
                        <div className=" cursor-pointer">
                            <p className="uppercase text-white text-xs font-bold leading-4">Adicionar ao pedido </p>
                            <span className="text-white text-xs font-normal leading-3"> R$ 27,00 </span>
                        </div>
                    </div>
                </div>
