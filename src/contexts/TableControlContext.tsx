...
import { iOrdersProductsData } from '../helpers/getOrdersProductsData';
import {
  tableReducer,
...
interface iTableContextProps {
  ...
  restaurant: iRestaurant['data']; // sssss
...
export default function TableContextProvider({
  ...
  async function createNewtable(cheirAmount: string, tableName: string) {
    ...
    const tableList = await api.get('api/table_control/' + restaurant.id);

    const tableAlreadyExists = tableList.data.find(
      (table: any) =>
