...
import { iProducts, iProductsWithFKData } from '../types/iProducts';

export const updateProductPriceByChangeType = ({
  changeType,
  products,
  changeAmount,
}: {
  changeAmount: number;
  products: iProducts['data'] | iProductsWithFKData[];
  changeType:
    | ''
    | 'increaseByAmount'
    | 'reduceByAmount'
        ...
        await supabase
          ...
          .update({
            price:
              ...
              Number(product.price) * (Number(changeAmount) / 100),
          })
          .eq('id', product.id);
      });
      break;
