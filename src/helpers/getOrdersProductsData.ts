...
export function getOrdersProductsData({
  ordersProducts,
  additionals,
  products,
  selects,
}: iGetOrdersProductsDataProps) {
  const orderProductData = ordersProducts.reduce(
    (acc: iOrdersProductsData[], item) => {
      const additionalsData = item.additionals_data as {
        quantity: number;
      ...
      const additionalsDataFiltered = additionalsData.reduce(
        (accAdd: iOrdersProductsData['additionalsData'], addData) => {
          const additionalFound = additionals.find(
            add => addData.additional_id === add.id
          );
          ...
          if (additionalFound) {
            return (accAdd = [
              ...accAdd,
              {
                ...
                totalPriceOfAdditionals:
                  addData.quantity * additionalFound.price,
              },
            ]);
          }
      ...
      const totalAdditionalsPriceByProduct = additionalsDataFiltered.reduce(
        (addAcc, addItem) => addAcc + addItem.totalPriceOfAdditionals,
        0
      );

      const selectsDataFiltered: ss = selectsData?.reduce(
        (sAcc: any, sItem) => {
          const findSelectById = selects.find(
            s => s.id == sItem.options[0].select_id
          );
